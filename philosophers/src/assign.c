...
t_philo *allocate_philos(const size_t size, t_locks *locks, t_intervals intervals) {
    ...
    while (i < size) {
        ...
        philos[i].table_size = size;  // Add this line
...
static void assign_forks(const t_table *table, t_philo *philo) {
    printf("Philosopher %zu will receive {right fork: %zu, left fork: %zu}\n",
    ...
    printf("Philosopher %zu has forks %zu (left) and %zu (right)\n", 
           philo->id, (philo->id + table->size - 1) % table->size, philo->id);
}
